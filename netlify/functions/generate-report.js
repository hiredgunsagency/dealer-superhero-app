...
exports.handler = async (event) => {
  ...
  const { dealer_id, report_month, report_year } = body;
  if (!dealer_id) return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'dealer_id required' }) };

  try {
    // Pull all dealer data from Supabase
    const [dealerRes, vendorsRes, competitorsRes, salesRes, vendorLeadsRes] = await Promise.all([
      ...
      sbGet(`sales_data?dealer_id=eq.${dealer_id}&sale_month=eq.${report_month || new Date().getMonth()+1}&sale_year=eq.${report_year || new Date().getFullYear()}`),
      sbGet(`vendor_leads?dealer_id=eq.${dealer_id}&attribution_month=eq.${report_month || new Date().getMonth()+1}&attribution_year=eq.${report_year || new Date().getFullYear()}&select=*,vendors(vendor_name,monthly_fee,category)`),
    ]);
  ...
  "actionPlan": [
    {
      ...
      "investmentDetail": "<specific detail — e.g. '2-3 hours/week from your marketing coordinator' or 'Budget $500-800/mo' or '$1,200/mo + 4hrs/week setup'>",
...
5. Check vendor category benchmarks — what should a ${dealer.brand || 'auto'} dealer's SEO vendor be delivering?
...
Be specific. Reference real data. Do not be generous with scores.`;
    ...
    const reportId = `${dealer_id}-${year}-${month}`;
    const saved = await sbPost('monthly_reports', {
      dealer_id,
      report_month: month,
      report_year: year,
